...
RObject runSAX(std::vector<float> orgData, int segmentSize, int alphabetSize, bool iSAX = false) {
  ...
  //return values. If iSAX is true, return the word and it's cardinality
  if (iSAX)
...
float *getTable(int alphabetSize) {
  //if the normal gaussian distribution for the chosen alphabetSize hasn't been created before. Create it now
  if(tableMap.find(alphabetSize) == tableMap.end()){
    float *tempTable = new float[alphabetSize - 1];
    double interval = 1 / (double)alphabetSize;
    int tile = 10; //tbd
    //compute normal gaussian distribution table
#pragma omp parallel for
    ...
    //save table in a map for easy access later
    tableMap.insert (std::pair<int,float*> (alphabetSize,tempTable));
    return tempTable;
  }
  //if the map exists return the table saved in the map
  else
...
void toSAX(std::vector<float> *data, std::vector<int> *SAXWord, std::vector<int> *card, int numBkPts){
  ...
  int tile = 4096;
  // iterating through PAA data, assign SAX symbol based on normal gaussian distribution table
#pragma omp parallel for
  for (int ii = 0; ii < dataSize; ii += tile) {
    for (int i = ii; i < ii + tile && i < dataSize; i++) {
      int counter = 0;
